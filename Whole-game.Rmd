...
chunks <- parsermd::rmd_subset(rmd, type_refs = "rmd_chunk")
...
# the new session. This doesn't work for foofactors because the package is
# deleted at the end of the doc. So we just hack knitr to not save or load
# the package info
assignInNamespace(
  "cache", ns = "knitr",
...
# being kind someone working interactively and locally
(owd <- getwd())
...
```{r use-git, eval = create, chunk_envvar = c(rlang_interactive = "FALSE")}
use_git()  
```
...
```{r gert-begin, echo = debug, comment = NA, include = FALSE, eval = create}
suppressPackageStartupMessages(library(gert))

git_config_set("user.name", "Whole Game")
git_config_set("user.email", "whole_game@example.com")
...
```{r post-git-file-list, eval = create}
...
:::downlit
```{r fbind-fodder, echo = FALSE, comment = NA}
fbind_fodder <- c(
  "fbind <- function(a, b) {",
  "  factor(c(as.character(a), as.character(b)))",
  "}"
...
```{r init-fbind, eval = create, chunk_envvar = c(rlang_interactive = "FALSE")}
use_r("fbind")
```
...
git_add(path("R", "fbind.R"))
git_commit("Add fbind()")
...
```{r commit-description, echo = debug, comment = NA, include = FALSE, eval = create}
git_add("DESCRIPTION")
...
```{r commit-license, echo = debug, comment = NA, include = FALSE, eval = create}
git_add(c("LICENSE", "LICENSE.md", ".Rbuildignore", "DESCRIPTION"))
git_commit("Add LICENSE")
```
...
:::downlit
```{r fbind-roxygen-header, echo = FALSE, comment = NA, eval = create}
...
```{r commit-fbind-roxygen-header, echo = debug, comment = NA, include = FALSE, eval = create}
git_add(file.path("R", "fbind.R"))
git_commit("Add roxygen header to document fbind()")
```
...
```{r commit-namespace, echo = debug, comment = NA, include = FALSE, eval = create}
git_add(c("DESCRIPTION", "NAMESPACE", path("man", "fbind.Rd")))
...
```{r commit-testthat-init, echo = debug, comment = NA, include = FALSE, eval = create}
...
:::downlit
```{r test-fbind-fodder, echo = FALSE, comment = NA, eval = create}
test_fodder <- c(
...
This tests that `fbind()` gives an expected result when combining two charachter vectors and a character vector and a factor.

```{r commit-fbind-test, echo = debug, comment = NA, eval = FALSE}
git_add(path("tests", "testthat", "test-fbind.R"))
git_commit("Test fbind()")
```
...
```{r test, eval = create, message = FALSE, R.options = list(testthat.default_reporter = testthat::ProgressReporter$new(update_interval = Inf))}
test()
```
...
```{r commit-forcats-imports, echo = debug, comment = NA, include = FALSE, eval = create}
git_add("DESCRIPTION")
git_commit("Import forcats")
```
...
:::downlit
```{r fcount-fodder, echo = FALSE, comment = NA}
fcount_fodder <- c(
...
```{r commit-fcount, echo = debug, comment = NA, include = FALSE, eval = create}
git_add(path("R", "fcount.R"))
git_commit("Add fcount()")
```
...
```{r commit-fcount-rd, echo = debug, comment = NA, include = FALSE, eval = create}
git_add(c("NAMESPACE", path("man", "fcount.Rd")))
git_commit("Document and export fcount()")
```
...
```{r use-readme-rmd, eval = create, chunk_envvar = c(rlang_interactive = "FALSE")}
use_readme_rmd()
```
...
```{r commit-rendered-readme, echo = debug, comment = NA, include = FALSE, eval = create}
git_add(c(".Rbuildignore", "README.Rmd", "README.md"))
git_commit("Write README.Rmd and render")
```

```{r final-push, include = FALSE, eval = FALSE}
# execute manually if you want to update the foofactors repo on github
# pre-requisite:
#   render of this with no existing cache and `where <- "tmp_user"`
library(gert)
proj_set("~/tmp/foofactors")
setwd(proj_get())
git_remote_add("https://github.com/jennybc/foofactors.git")
git_push(force = TRUE)
```
